...
from backend.models.poetry import PoetryDungeonState
from backend.services.poetry_dungeon import (
    start_dungeon,
...
router = APIRouter(prefix="/api/dungeon/poetry", tags=["poetry_dungeon"])
...
@router.post("/start")
async def start_poetry_dungeon():
    """开始吟诗作对关卡，返回 session_id 和题目"""
    state = await start_dungeon()
    return {
        "session_id": state.session_id,
        "topic": {
            "theme": state.topic.theme,
            "constraint": state.topic.constraint,
            "full_prompt": state.topic.full_prompt
        }
    }
...
@router.websocket("/ws/dungeon/poetry/{session_id}")
async def poetry_websocket(websocket: WebSocket, session_id: str):
    """WebSocket 连接处理"""
        ...
        try:
            ...
            if data.get("type") == "player_answer":
                ...
                if state and state.result:
                    await websocket.send_json({
                        "type": "result",
                        "result": state.result,  # "win" 或 "lose"
                        "message": "恭喜你赢得了比赛！" if state.result == "win" else "很遗憾，对手更胜一筹。"
                    })
            else:
                await websocket.send_json({
                    "type": "error",
